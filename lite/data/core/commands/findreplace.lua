...
local function has_unique_selection()
    ...
    for idx, line1, col1, line2, col2 in doc():get_selections(true, true) do
        ...
        if text ~= nil and text ~= selection then
            return false
        end
        text = selection
    end
...
local function is_in_selection(line, col, l1, c1, l2, c2)
    ...
    if line == l1 and col <= c1 then
        return false
    end
    if line == l2 and col > c2 then
        return false
    end
    return true
end
...
local function is_in_any_selection(line, col)
    for idx, l1, c1, l2, c2 in doc():get_selections(true, false) do
        if is_in_selection(line, col, l1, c1, l2, c2) then
            return true
        end
    end
    return false
...
local function select_add_next(all)
    ...
    for idx, l1, c1, l2, c2 in doc():get_selections(true, true) do
        ...
        repeat
            l1, c1, l2, c2 = search.find(doc(), l2, c2, text, {
                wrap = true
            })
            if l1 == il1 and c1 == ic1 then
                break
            end
...
local function select_next(reverse)
    ...
    local text = doc():get_text(l1, c1, l2, c2)
    if reverse then
        l1, c1, l2, c2 = search.find(doc(), l1, c1, text, {
            wrap = true,
            reverse = true
        })
    else
...
command.add(has_unique_selection, {
    ...
    ["find-replace:select-add-all"] = function()
        select_add_next(true)
    end
})
