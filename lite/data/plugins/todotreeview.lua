...
"@todo", "@fixme", "@testme", "@leak"} -- neko hack

-- Paths or files to be ignored
config.todo_ignore_paths = {"../source/engine", -- neko hack
"..\\source\\engine" -- neko hack
}
...
config.todo_allowed_extensions = '.h.c.m.hh.cc.hpp.cpp.cxx.lua.py.cs.vs.fs.bat.' -- neko hack
...
local function find_file_todos(t, filename)
    ...
    -- neko hack: optimized loops: early exit if quicksearch fails
...
function TodoTreeView:on_mouse_pressed(button, x, y)
    if not self.hovered_item then
        return
    elseif self.hovered_item.type == "file" or self.hovered_item.type == "group" then
...
keymap.add {
    ["ctrl+shift+e"] = "todotreeview:expand-items"
}
keymap.add {
    ["ctrl+shift+h"] = "todotreeview:hide-items"
}
